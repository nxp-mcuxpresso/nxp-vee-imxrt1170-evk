...
public class TouchAnimation {
	...
	public TouchAnimation(int x, int y) {
		 ...
		 * screen.
		 */
		float scale = 1.4f - ((float) y / (float) d.getHeight());
		this.matrix = new Matrix();
		this.matrix.setScale(scale, scale);
		this.matrix.postTranslate(x - (scale * AnimatedMascot.demo.touchImage.getWidth() / 2),
				y - (scale * AnimatedMascot.demo.touchImage.getHeight() / 2));
