...
void DISPLAY_VGLITE_start_operation(bool wakeup_graphics_engine) {
	...
	// VG drawing has been added to the GPU commands list: ask to sudmit VG operation
	vg_lite_flush();
...
uint32_t DISPLAY_VGLITE_premultiply(uint32_t color) {
	...
	return DISPLAY_VGLITE_premultiply_alpha(color, alpha);
}
...
void DISPLAY_VGLITE_IRQHandler(void) {
	...
	case IRQ_WAKEUP_TASK :{
		...
		// DISPLAY_VGLITE_start_operation()) to draw the next steps of this "big" drawing (text, raw image,
		// bvi, etc.).
