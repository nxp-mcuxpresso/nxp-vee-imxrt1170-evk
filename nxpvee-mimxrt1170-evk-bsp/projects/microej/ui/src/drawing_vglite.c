...
/*
 * @brief Adjusts line direction according to steering coefficient. The lines are always drawn
 * in the same direction: x positive and y positive or negative. The given matrix is reset and
 * translated to the start point.
 *
 * @param[in|out] line first point X coordinate
 * @param[in|out] line first point Y coordinate
 * @param[in|out] line last point X coordinate
 * @param[in|out] line last point Y coordinate
 * @param[in] the matrix to translate
 *
 * @return the Y direction: 1 or -1.
 */
static int __adjust_line(
		int* x1,
		int* y1,
		int* x2,
		int* y2,
...
DRAWING_Status DW_DRAWING_drawThickFadedPoint(
	...
	if ((fade > 1) // No radial gradient with hardware
		...
		jint l_thickness = thickness == 0 ? fade : thickness;
		float dx = x;
		float dy = y;
		float ft = l_thickness;
		ft /= 2.f;

		ret = __fill_ellipse(gc, dx - ft, dy - ft, l_thickness, l_thickness);
	}
	return ret;
}
...
DRAWING_Status DW_DRAWING_drawThickFadedCircle(
	...
	if ((fade > 1) // No radial gradient with hardware
		...
		jint l_thickness = thickness == 0 ? fade : thickness;
		diameter_out = diameter + l_thickness;
		diameter_in = diameter - l_thickness;
		float dx = x;
		float dy = y;
		float ft = l_thickness;
		ft /= 2.f;
...
DRAWING_Status DW_DRAWING_drawThickFadedEllipse(
	...
	if ((fade > 1) // No radial gradient with hardware
		...
		jint l_thickness = thickness == 0 ? fade : thickness;
		diameter_out_w = width + l_thickness;
		diameter_out_h = height + l_thickness;
		diameter_in_w = width - l_thickness;
		diameter_in_h = height - l_thickness;
...
// See the header file for the function documentation
DRAWING_Status UI_DRAWING_copyImage(
		MICROUI_GraphicsContext* gc,
		MICROUI_Image* img,
		jint regionX,
		jint regionY,
	...
	return (img == &gc->image) ?
			...
			UI_DRAWING_drawRegion(gc, regionX, regionY, width, height, x, y, 0xff)
			// no overlap: draw image as usual
			: UI_DRAWING_drawImage(gc, img, regionX, regionY, width, height, x, y, 0xff);
}
...
DRAWING_Status DW_DRAWING_drawScaledImageBilinear(
	...
	if(!__configure_source(&source_buffer, img)){
		...
		ret = __scale_image(gc, img, &source_buffer, x, y, factorX, factorY, alpha, filter);
	}
...
static DRAWING_Status __draw_ellipse_arc(
	...
	if (__check_clip(gc, x - 1, y - 1, x + diameter_w, y + diameter_h, false)) {
...
static DRAWING_Status __draw_ellipse(
	...
	// Check if there is something to draw and clip drawing limits
	// +/- 1: consider 1 pixel of antialiasing
	if (__check_clip(gc, (int)(x - 1), (int)(y - 1), (int)(x + diameter_out_w), (int)(y + diameter_out_h), false)) {
...
static DRAWING_Status __fill_ellipse(
	...
	if (__check_clip(	gc,
		...
		shape_vg_path.path_length = sizeof(__shape_paths.ellipse);
		(void)VGLITE_PATH_compute_filled_ellipse(&shape_vg_path, (int)radius_w, (int)radius_h, &matrix);
...
static int __adjust_line(
	...
	if (*x1 > *x2){
		// swap points: always start at (x1,y1)
		int temp = *x1;
		*x1 = *x2;
		*x2 = temp;
		temp = *y1;
...
static DRAWING_Status __draw_line(
	...
	int xe = x2;
	int ye = y2;
	int yi = __adjust_line(&xs, &ys, &xe, &ye, &matrix);

	// Compute the thick shape path
	shape_vg_path.path = &__shape_paths.line;
	shape_vg_path.path_length = sizeof(__shape_paths.line);
	(void)VGLITE_PATH_compute_line(&shape_vg_path, xe - xs, ye - ys, 1, yi);
...
static DRAWING_Status __thick_line(
	...
	if (__check_clip(
		...
		(void)VGLITE_PATH_compute_thick_shape_line(
				&shape_vg_path,
				(int)(DRAWING_SCALE_FACTOR * (xe - xs + 1)),
				(int)(DRAWING_SCALE_FACTOR * (ye - ys + yi)),
...
static DRAWING_Status __scale_image(
	...
	if (__check_clip(
		...
		src->height = img->height; // Workaround for the image size. See __buffer_set_address_and_size().
		src->width = img->width;
...
static vg_lite_color_t __get_vglite_color(MICROUI_GraphicsContext* gc, MICROUI_Image* img, jint alpha) {
	...
	case MICROUI_IMAGE_FORMAT_A8:
		fc = DISPLAY_VGLITE_premultiply_alpha(gc->foreground_color, alpha);
		break;
	default:
		fc = alpha * 0x01010101;
		break;
...
static bool __prepare_gpu_draw_image(MICROUI_GraphicsContext* gc, MICROUI_Image* img, jint x_src, jint y_src, jint width, jint height, jint x_dest, jint y_dest, jint alpha, void** target, vg_lite_color_t* color, vg_lite_matrix_t* matrix, uint32_t* blit_rect){
	...
	if(__configure_source(&source_buffer, img) && __check_clip(gc, x_dest, y_dest, x_dest + width - 1, y_dest + height - 1, true)) {
