...
int VGLITE_PATH_compute_rounded_rectangle(
	...
	int radius_width = arc_width > 0 ? arc_width / 2 : 0;
	int radius_height = arc_height > 0 ? arc_height / 2 : 0;
...
// See the header file for the function documentation
inline void VGLITE_PATH_update_color(vg_lite_color_t* color) {
	// premultiply color
	*color =  DISPLAY_VGLITE_premultiply(*color);
}

// See the header file for the function documentation
void VGLITE_PATH_update_gradient(vg_lite_linear_gradient_t* gradient) {

	// premultiply each color
	uint32_t count = gradient->count;
	uint32_t* colors = gradient->colors;
	for (uint32_t i = 0; i < count; i++) {
		colors[i] = DISPLAY_VGLITE_premultiply(colors[i]);
	}
...
int __update_path(vg_lite_path_t *path,
	...
	if (first_path) {
		// use vg-lite API to reset the path
