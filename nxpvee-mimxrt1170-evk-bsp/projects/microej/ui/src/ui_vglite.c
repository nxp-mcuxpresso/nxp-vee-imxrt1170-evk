...
bool UI_VGLITE_configure_source(vg_lite_buffer_t *buffer, MICROUI_Image* image) {
	...
	if (!LLUI_DISPLAY_isLCD(image) || (LLUI_DISPLAY_IMPL_getNewImageStrideInBytes(image->format, image->width, image->height, stride) == stride)){
...
uint32_t UI_DRAWING_getNewImageStrideInBytes(jbyte image_format, uint32_t image_width, uint32_t image_height, uint32_t default_stride) {
	...
	if (!LLUI_DISPLAY_isCustomFormat(image_format)) {
		/*
		...
		ret = (bpp >= (uint32_t)8) ? ALIGN(image_width, (uint32_t)16) * (bpp / (uint32_t)8) : ALIGN(image_width, (uint32_t)8);
	}
	else {
		ret = default_stride;
	}
